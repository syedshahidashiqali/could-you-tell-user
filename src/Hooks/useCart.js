...
function useCart() {
  ...
  const updateStorage = (_data = []) => {
    dispatch(setCart(_data));
  ...
  const checkAttribs = (attribs1, attribs2) => {
    ...
    attribs1.forEach((attrib) => {
      const attrib2Obj = attribs2.find(
        (e) => e.attributeId === attrib.attributeId
      );
  ...
  const updateCart = (value, productData, increaseQtyIfExists = false) => {
    ...
    let cartIndex = newCartData.findIndex(
      (item) =>
    ...
    if (cartIndex >= 0) {
      ...
      if (!increaseQtyIfExists) cartItem.qty = value;
      else cartItem.qty += value;
