...
export default function ProductDetail() {
  let { id } = useParams();
  const [selectionType, setSelectionType] = useState("unselect");
  ...
  const setSelectedAttribute = async (data) => {
    ...
    let attributeIndex = await newAttributes.findIndex(
      (item) => data?.attributeId == item?.attributeId
    );
    console.log(attributeIndex);
    if (attributeIndex >= 0) {
      if (data?.value == undefined) newAttributes.splice(attributeIndex, 1);
      else newAttributes[attributeIndex].value = data?.value;
    } else {
    ...
    <div className="container ">
      <div className="row py-lg-5 pt-md-5 pb-2 align-items-start justify-content-around">
        ...
        <div className="col-lg-6 col-12">
          ...
          <h4 className="product-type text-white text-uppercase pb-3 fs-18">
            Product Type: abc
          </h4>
          <ul className="list-inline rating my-3">
            <li className="list-inline-item">
              <span className="text-white">
                {product.total_ratings} Ratings &amp; {product.total_ratings}{" "}
                Reviews
              </span>
            </li>
          </ul>
          ...
          <button
            onClick={(e) => updateCartItem()}
            type="button"
            className="gold-btn-solid d-inline-block my-4 eq-width-btn"
          >
            Add to Cart
          </button>
        </div>
      ...
      <div className="row my-lg-5 text-white">
        ...
        <div className="col-12 mt-lg-5">
          <h3 className="faq-title fs-30 font-weight-light">
            Rating &amp; Review of {product?.title}
          </h3>
            ...
            <div className="col-xl-6 col-lg-8">
              ...
              {ratings?.map((rate, rateIndex) => (
                <div key={rateIndex} className="rating-status">
                  ...
                  <p className="ends-on-percent">
                    {getRatingPercentage(product.total_ratings, rate.count)}
                  </p>
                </div>
              ))}
            </div>
          ...
          <div className="review-comments mt-5">
            ...
            {reviews?.data?.map((review, reviewIndex) => (
              <div
                key={reviewIndex}
                className="row align-items-start justify-content-between my-md-5"
              >
                ...
                <div className="col-lg-9 col-md-10">
                  <h5 className="goboldthin font-dark">
                    {review?.user?.name} --- {format_date(review.createdAt)}
                  </h5>
